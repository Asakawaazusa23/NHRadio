...
class ReplaceWorker(QThread):
    ...
    def run(self):
        ...
        for idx, (file_path, meta) in sorted(self.replace_map.items()):
            ...
            try:
                raw, sc, full_fsb5 = self.engine.encode_song(file_path)
                new_data[idx] = (raw, sc, full_fsb5)
        ...
        if new_data:
            ...
            try:
                ...
                updates = {idx: (meta[0], meta[1], new_data[idx][1]) for idx, (_, meta) in self.replace_map.items() if idx in new_data}
...
class MainWindow(QMainWindow):
    ...
    def _on_bank_changed(self, idx):
        ...
        self.log(f" 从XML重建 {bank_name} 歌单映射...")
        sm = self.engine.auto_generate_song_map(bank_name)
        self.log(f" 已加载 {len(sm)} 首歌曲映射", "SUCCESS")
